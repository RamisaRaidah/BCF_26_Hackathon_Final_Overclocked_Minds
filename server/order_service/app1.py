...
# ORDER_PORT = int(os.getenv("PORT", "8000"))
...
#     # Track only Order Service response latency (all endpoints)
#     try:
...
@app.post("/orders/<int:order_id>/ship")
def ship_order(order_id: int):
    ...
    # timeout / 5xx / network error: Schrödinger resolver
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=ORDER_PORT, debug=True)